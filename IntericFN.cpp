...
std::string name = E("Spoofer").decrypt(); // application name. right above the blurred text aka the secret on the licenses tab among other tabs
std::string ownerid = E("TuMpSxJL6t").decrypt(); // ownerid, found in account settings. click your profile picture on top right of dashboard and then account settings.
std::string secret = E("316905e9e83a7701a2d9bf67c9b87ba6b328a4e45574b8f68b6b6060e202bc6f").decrypt(); // app secret, the blurred text on licenses tab and other tabs
...

{
	...
	std::cout << ("\n");
	LI_FN(Sleep)(1500);
	std::cout << slowly_printing_string{ " [+] Have Fun <3 ", 50};
	std::cout << ("\n");
	LI_FN(Sleep)(1500);
	std::cout << slowly_printing_string{ " [+] Dont forget to Vouch:) ",50};
	LI_FN(Sleep)(1500);
	std::cout << ("\n");
	std::cout << ("\n");
	std::cout << ("\n");
	std::cout << ("\n");
	std::cout << slowly_printing_string{ " [+] MADE BY ROCKYLAMA ", 50};
	std::cout << ("\n");
	std::cout << ("\n");
	std::cout << ("\n");
	std::cout << ("\n");
...
void render() {
	...
	ImGui::Text("Welcome to The Anarchy(<3)");
	...
	if (ShowMenu == true)
	{
		ImGui::Begin("The Anarchy | Version Beta", NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar);
		if (ShowMenu)
		{
			//Basic Sizing
			...
			if (tabs == 0)
			{
				...
				ImGui::Spacing();
				ImGui::CustomCheckbox("Circle FOV", "Draws the FOV Circle", &fovcircle);
				ImGui::Spacing();
			...
			if (tabs == 1)
			{
				...
				ImGui::Spacing();
				ImGui::CustomCheckbox("Filled Box", "Fills the box with custom color", &Fillbox);
				ImGui::Spacing();
				ImGui::CustomCheckbox("Skeleton(Updating)", "Draws a skeleton onto the player", &SKELETONesp);
				ImGui::Spacing();
				ImGui::CustomCheckbox("Centered Snaplines", "Draws a line from FOV", &lineheadesp);
				ImGui::Spacing();
			   ...
			   //  ImGui::InputTextWithHint("Text input", "Text Example", inputtext, 64);
				...
				if (tabs == 2)
				{
					...
					ImGui::Text("Developer : RockyLama|Have Fun<3");
					ImGui::Spacing();
					ImGui::Text("discord.gg/DbwgZJqVEF");
					ImGui::Spacing();
